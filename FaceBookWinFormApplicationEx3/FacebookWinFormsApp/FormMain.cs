...
namespace BasicFacebookFeatures
{
    public partial class FormMain : Form ,IFacebookObserver
    {
        ...
        private void initUserPosts()
        {
            ...
            try
            {
                ...
                Invoke(new Action(() =>
                {
                    ...
                    IEnumerable<Post> postsCollection = new PostCollection(posts);
                    IEnumerator<Post> postIterator = postsCollection.GetEnumerator();
                    while (postIterator.MoveNext())
                    {
                        string postMessage = postIterator.Current.Message;
                        if (postMessage != null)
                        {
                            listBoxPosts.Items.Add(
                                $"{postIterator.Current.CreatedTime?.ToString("MM/dd/yyyy HH:mm")} : {postMessage}");
                        }
                    }
                }));
            }
        ...
        private void downloadAlbum(string i_SaveAlbumPath)
        {
            ...
            DownloadAlbumManager.DownloadAlbumInstance.m_AlbumToDownload = chosenAlbum;
            DownloadAlbumManager.DownloadAlbumInstance.DownloadAlbum(progressBarDownload,
                i_SaveAlbumPath);    
        }
        ...
        private void initSortStrategyLOptions()
        {
            checkedListBoxSortOptions.Items.Clear();
            checkedListBoxSortOptions.Items.Add(eStrategyOptions.SortByCommentsAmountAscending);
            checkedListBoxSortOptions.Items.Add(eStrategyOptions.SortByCommentsAmountDescending);
            checkedListBoxSortOptions.Items.Add(eStrategyOptions.SortByCreationTimeAscending);
            checkedListBoxSortOptions.Items.Add(eStrategyOptions.SortByCreationTimeDescending);
            checkedListBoxSortOptions.SetItemChecked(0, true);
            checkedListBoxSortOptions.SetSelected(0, true);
        }
        private void buttonDownloadAlbum_Click(object sender, EventArgs e)
        {
            hideAllPanelsOfUserSelection(panelDownloadPhotos);
            initSortStrategyLOptions();
        ...
        private void buttonDownload_Click(object sender, EventArgs e)
        {
            ...
            eStrategyOptions selectedItem = (eStrategyOptions)checkedListBoxSortOptions.SelectedItem;
            DownloadAlbumManager.CreateStrategy(selectedItem);
        ...
        private void checkedListBoxSortOptions_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            for (int index = 0; index < checkedListBoxSortOptions.Items.Count; index++)
            {
                if (index != e.Index)
                {
                    checkedListBoxSortOptions.SetItemChecked(index, false);
                }
            }
        }  
    }
